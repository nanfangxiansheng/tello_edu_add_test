...
class TelloUI:
    ...
    def __init__(self,tello,outputpath):
        ...
        self.detections = np.zeros((3, 5), dtype=np.float32)
       ...
       # self.panel_for_pose_handle_show = None
        ...
        self.btn_yolo = tki.Button(self.root, text="yolo Recognition Status: Off", 
                                   command=self.yolodetect)
        self.btn_yolo.pack(side="bottom", fill="both",
                           expand="yes", padx=10, pady=5)
    ...
    def yolodetect(self):
        if self.yolo_mode is False:
            self.yolo_mode = True
            self.btn_yolo.config(text='yolo Recognition Status: On')
        else:
            self.yolo_mode = False
            self.btn_yolo.config(text='yolo Recognition Status: Off')
